...
import { GIFT_KEYS, MYSTERY_POOL } from '../catalog.js';
...
router.post(
  ...
  ah(async (req, res) => {
    if (!GIFT_KEYS.includes(req.valid.giftId)) throw badRequest('Unknown gift');
    const partner = await partnerOf(req.user);
    if (!partner) throw badRequest('Your person hasn’t joined yet');
    const gift = await prisma.gift.create({ data: { id: req.valid.id, coupleId: req.coupleId, senderId: req.user.id, recipientId: partner.id, giftKey: req.valid.giftId, message: req.valid.message ?? '' } });
    // The server — not the sender's client — tells the recipient, so every
    // gift that arrives is one that was actually saved.
